...
from transforms import read_split_data
import random
import numpy as np
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        for epoch in range(engine.state.epoch, config.nepochs+1):
            ...
            for idx in pbar:
                ...
                criterion1 = nn.CrossEntropyLoss(reduction='mean', ignore_index=255)
                oout = model(imgs, modal_xs, gts)
                loss = criterion(oout, gts.long())
            ...
            if (epoch%1==0 and epoch>int(config.checkpoint_start_epoch)) or epoch==1:
                ...
                # 下方代码是TensorBoard记录训练损失和模型评估和保存
                # if engine.distributed:
                #     with torch.no_grad():
                #         model.eval()
